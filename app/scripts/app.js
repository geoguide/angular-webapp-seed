...
(function() {
  ...
  webapp.config(function($routeProvider, ENV) {
    ...
    $routeProvider.when('/', {
      ...
      controllerAs: 'dr'
    }).when('/providers/:id/provider-notes', {
      tab: 'provider-notes',
      templateUrl: 'views/provider-notes.html',
      access: {
        requiredLogin: true
      },
      controller: 'ProviderNotesCtrl',
      controllerAs: 'dr'
    }).when('/offers/:id', {
      tab: 'offers',
  ...
  webapp.config(function Config($httpProvider, jwtInterceptorProvider) {
    jwtInterceptorProvider.tokenGetter = ['jwtHelper', '$http', 'localStorageService', 'API_URL', '$location',
      function(jwtHelper, $http, localStorageService, API_URL, $location) {
